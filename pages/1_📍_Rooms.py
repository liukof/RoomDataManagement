            ...
            if up_file and st.button("🚀 Sync Rooms"):
                ...
                for _, row in df_up.iterrows():
                    ...
                    bulk_data.append({
                        ...
                        "parameters": params
                        # Nota: L'area non viene importata da Excel perché l'autorità è Revit
                    })
...
# AGGIUNTO: Selezioniamo esplicitamente 'area'
rooms_resp = supabase.table("rooms").select("*").eq("project_id", project_id).order("room_number").execute()

if rooms_resp.data:
    flat_data = []
    for r in rooms_resp.data:
        # AGGIUNTO: 'Area' inserita nel dizionario flat
        row = {
            "id": int(r["id"]), 
            "Number": r["room_number"], 
            "Name": r["room_name_planned"],
            "Area (mq)": float(r.get("area") or 0) # Convertiamo in float per la tabella
        }
    ...
    # AGGIUNTO: Configurazione colonne per rendere l'Area non editabile (autorità Revit)
    ed_rooms = st.data_editor(
        ...
        column_config={
            "id": None, 
            "Area (mq)": st.column_config.NumberColumn(format="%.2f m²", help="Dato sincronizzato da Revit (Sola Lettura)", disabled=True)
        }
    )
    ...
    if catalog:
        ...
        with st.form("bulk_item"):
            ...
            if st.form_submit_button("🚀 Add to Filtered Set"):
                ...
                st.success(f"Added to {len(bulk)} rooms!"); st.rerun()mapped_params = [m['db_column_name'] for m in maps_resp.data]
    ...
    if catalog:
        ...
        with st.form("bulk_item"):
            ...
            if st.form_submit_button("🚀 Add to Filtered Set"):
                bulk = [{"room_id": int(rid), "item_id": item_opt[t_item], "quantity": int(t_qty)} for rid in df_filtered['id'].tolist()]
                supabase.table("room_items").insert(bulk).execute()
                st.success(f"Added to {len(bulk)} rooms!"); st.rerun()
